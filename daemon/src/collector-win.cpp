...
namespace
{
  ...
  float getTotalDiskSize()
  {
    ...
    drive_str.push_back(':');
    std::cout << drive_str << '\n';
    HANDLE h_drive = CreateFileA(drive_str.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    ...
    bool success
        = DeviceIoControl(h_drive, IOCTL_DISK_GET_LENGTH_INFO, NULL, 0, &disk_length, sizeof(disk_length), NULL, NULL);
    if (!success)
    {
